...
window.addEventListener('load', async () => {
    ...
    if (isAuthenticated) {
        ...
        const authenticationApiAccessToken = await auth0Client.getTokenSilently();

        const apiResponse = await window.fetch(`https://${auth0ClientOptions.domain}/userinfo`, {
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${authenticationApiAccessToken}`
            }
        });
        ...
        if (pageContents.length) {
            ...
            pageContents[0].innerHTML = `
                ...
                <pre>${authenticationApiAccessToken}</pre>
